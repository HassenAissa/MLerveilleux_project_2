...
def run_query(query,date, model, tokenizer, config, prompt_frame,max_answer_len = 100, device="cuda", temperature=1.0):
    ...
    #prompt_frame = " "#TODO: change this before productiob
    prompt_frame_pad = tokenizer.encode_ordinary(prompt_frame)
    query_len = len(query_tokens)
    query_tokens = tokenizer.encode_ordinary(" ")*(ctx_window_size - query_len - len(prompt_frame_pad)) + prompt_frame_pad + query_tokens
    #print("len frame",len(prompt_frame_pad))
    #print("Send tokens ",query_tokens)
    ...
    while new_token == None or (answer_count < max_answer_len and new_token.item() != tokenizer.eot_token):
        ...
        date_torch = None
        if date is not None:
            date_torch = torch.tensor([date]).to(device)
        output = model(query_tokens, date_torch, get_logits=True, moe=config.moe)
